    ...
    <div className={`flex items-center justify-center lg:px-0 ${isBigSize ?"h-[clamp(93px,43.6vw,855px)] lg:h-[clamp(93px,23.6vw,455px)]" :"h-[clamp(93px,44.6vw,835px)] lg:h-[clamp(93px,22.6vw,435px)]"}`}>
      <Link href={`/tours/${slug}`} className="relative w-full h-full rounded-3xl overflow-hidden">
        ...
        <div className="z-30 w-3/4">
          ...
          <div className={`font-extrabold ${isBigSize ? 'text-[clamp(13.24px,6.2vw,101.44px)] lg:text-[clamp(26.44px,6.4vw,122.88px)]':'text-[clamp(13.24px,5.2vw,91.44px)] lg:text-[clamp(13.24px,3.2vw,61.44px)]' } uppercase mb-3 multi-line-truncate multi-line-truncate-3 text-left ${isBigSize ? 'lg:leading-[clamp(10px,5vw,150px)]' : 'lg:leading-[clamp(5.4px,2.8vw,80.24px)]'}  max-w-full`}>{title}</div>
        ...
        <div className="absolute inset-0 lg:translate-y-0 flex items-center justify-end  w-full">
          <svg
            ...
            className={`${ isBigSize?'w-[300px]':'w-[230px] sm:w-[270px] md:w-[350px] lg:w-[190px] xl:w-[250px] 2xl:w-[280px]'} h-auto`}
          >
            <path
