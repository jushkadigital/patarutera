...
import type { Destination, Media, Paquete, Tour } from "@/cms-types";
import Link from "next/link";

import RichText from "./RichText";
import { CardPaqueteData } from "./cardPaquete";

export interface CardTourData {
  id: number | string;
  title?: string | null;
  slug?: string | null;
  miniDescription?: Tour["miniDescription"] | null;
  ...
  iconMaxPassengers?: Tour["iconMaxPassengers"] | null;
  maxPassengers?: number | null;
  iconDifficulty?: Tour["iconDifficulty"] | null;
...
type CardBothData = CardTourData | CardPaqueteData

interface CardBothProps {
  unitData: CardBothData;
  mode?: "grid" | "list";
}
...
function getTourHref(unitData: CardBothData): string {
  return unitData.slug ? `/pe/tours/${unitData.slug}` : "/pe/tours";
}

function getDestinationName(unitData: CardBothData): string {
...
function getPrice(unitData: CardBothData): number {
...
function getDifficulty(unitData: CardBothData): keyof typeof trad {
...
function renderTourImage(unitData: CardBothData, className: string) {
...
function renderIcon(
  media: Tour["iconDifficulty"] | Tour["iconMaxPassengers"] | Paquete["iconDifficulty"] | Paquete["iconMaxPassengers"] | null | undefined,
  fallbackSrc: string,
  className: string,
) {
  if (isMedia(media)) {
...
function renderDescription(unitData: CardBothData, className: string) {
...
export default function CardBoth({ unitData, mode = "list" }: CardBothProps) {
  const href = getTourHref(unitData);
