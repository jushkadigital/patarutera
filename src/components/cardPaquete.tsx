...
export type CardPaqueteData = Pick<Paquete, 'id' | 'title' | 'miniDescription' | 'featuredImage' | 'Desde' | 'price' | 'Person desc' | 'iconMaxPassengers' | 'maxPassengers' | 'iconDifficulty' | 'difficulty' | 'slug' | 'destinos' | 'medusaId'> & {
  priceMedusa: {
    amount: number;
    currency: string;
  } | null;
  ...
  if (mode === "grid") {
    return (
      <Card className="w-full max-w-[435px] mx-auto overflow-hidden rounded-3xl shadow-lg py-0 gap-1! h-[clamp(0px,166vw,1341.2px)] sm:h-[clamp(190.44px,74vw,883.2px)] lg:h-[clamp(190.44px,46vw,883.2px)] group hover:shadow-2xl transition-all duration-500 hover:shadow-blue-500/25">
        <Link href={`/pe/paquetes/${unitData.slug}`} className="h-[63%]">
        ...
        <CardContent className="px-4 sm:px-6 h-[37%]">
          {/* title */}
          <Link href={`/pe/paquetes/${unitData.slug}`}>
          ...
          <div className="w-full flex flex-row mt-[clamp(0px,4.1vw,42.04px)] sm:mt-[clamp(0px,1.5vw,42.04px)] lg:mt-[clamp(4.6px,1.13vw,21.76px)]">
            ...
            <Link href={`/pe/paquetes/${unitData.slug}`} className="w-full flex justify-center cursor-pointer w-1/2">
    ...
    return (
      <Card className="overflow-hidden shadow-lg w-full rounded-3xl gap-1! flex flex-row py-0 md:h-[clamp(191px,25.66vw,558px)] lg:h-[clamp(191px,18.66vw,358px)]  group hover:shadow-2xl transition-all duration-500 hover:shadow-blue-500/25">
        ...
        <Link href={`/pe/paquetes/${unitData.slug}`} className="w-1/3">
        ...
        <div className="w-1/3 p-1 sm:p-1 flex flex-col justify-center">
          ...
          <Link href={`/pe/paquetes/${unitData.slug}`} className="w-full flex justify-center">
            <Button className="w-fit h-fit bg-[#3eae64] hover:bg-[#35a058] text-white font-semibold  py-[clamp(0px,1.20vw,12.36px)] sm:py-[clamp(0px,0.70vw,12.36px)] lg:py-[clamp(1.38px,0.3vw,6.4px)] px-[clamp(0px,2.89vw,29.68px)]  sm:px-[clamp(0px,1.49vw,29.68px)] lg:px-[clamp(3.31px,0.8vw,15.36px)]  text-[clamp(0px,3.86vw,39.57px)] sm:text-[clamp(0px,1.86vw,39.57px)] lg:text-[clamp(4.4px,1vw,20.48px)] rounded-full cursor-pointer ">
              Ver Detalles
            </Button>
