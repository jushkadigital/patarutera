...
export const retrieveCustomer2 =
...
export const retrieveCustomer3 =
  async (): Promise<HttpTypes.StoreCustomer | null> => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/store/customers/me?fields=*orders`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        console.error(
          `Error retrieveCustomer: ${response.status} ${response.statusText}`,
        );
        return null;
      }
...
export const retrieveCustomer6 =
  async (): Promise<HttpTypes.StoreCustomer | null> => {
    try {
      const cookieStore = await cookies();
